...
const Checkout = () => {
  ...
  const createOrder = async (paymentMethod: "razorpay" | "stripe" | "cod") => {
    ...
    try {
      const { data } = await axios.post(`${restaurantService}/api/order/new`, { paymentMethod, addressId: selectedAddressId }, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      return data;
  ...
  const payWithRazorpay = async () => {
    try {
      ...
      if (!order) return;
      const { data } = await axios.post(`${utilsService}/api/payment/create`, { orderId: order.orderId });
  ...
  const payWithStripe = async () => {
    try {
      ...
      if (!order) return;
      await stripePromise;
      const { data } = await axios.post(`${utilsService}/api/payment/stripe/create`, { orderId: order.orderId });
