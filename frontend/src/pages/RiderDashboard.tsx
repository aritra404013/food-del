...
const RiderDashboard = () => {
  ...
  const fetchProfile = async () => {
    try {
      const { data } = await axios.get(`${riderService}/api/rider/myprofile`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const fetchCurrentOrder = async () => {
    try {
      const { data } = await axios.get(`${riderService}/api/rider/order/current`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const toggleAvailability = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      try {
        await axios.patch(`${riderService}/api/rider/toggle`, { isAvailble: !profile?.isAvailble, latitude: pos.coords.latitude, longitude: pos.coords.longitude }, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const handleSubmit = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      ...
      fd.append("phoneNumber", phoneNumber); fd.append("aadharNumber", aadharNumber);
      fd.append("drivingLicenseNumber", drivingLicenseNumber);
      fd.append("latitude", pos.coords.latitude.toString());
      fd.append("longitude", pos.coords.longitude.toString());
      if (image) fd.append("file", image);
      try {
        const { data } = await axios.post(`${riderService}/api/rider/new`, fd, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
